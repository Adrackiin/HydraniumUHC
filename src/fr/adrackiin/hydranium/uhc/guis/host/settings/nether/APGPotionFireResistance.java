...
public class APGPotionFireResistance extends APGuiListener implements APGCloseCustom {

    private APGui gui;
    private final Settings settings = Game.getGame().getSettings();
    private final HashMap<Byte, Boolean> back = new HashMap<>();
    private final PotionType type = PotionType.FIRE_RESISTANCE;

    public APGPotionFireResistance() {
        ...
        this.gui = new APGui("§cPotion de Résistance au feu", (byte)3, new APGuiItem[]{
                deactivate.get((byte)1), activate.get((byte)7),
                deactivate.get((byte)10), activate.get((byte)16),
                cancel.get((byte)21),
                validate.get((byte)23),
                new APGuiItem(Material.REDSTONE, (short)0, (byte)1, (byte)4, get((byte)1), null),
                new APGuiItem(Material.SULPHUR, (short)0, (byte)1, (byte)13, get((byte)2), null),
        }, parent, false, false);
    ...
    private String[] getDescParent(){
        return new String[]{"",
                "§6État: §aActivée", "", get((byte) 1), get((byte) 2)};
    }
    ...
    private void validate(){
        ...
        gui.getParent().getItem((byte)31).setDescription(Collections.singletonList(getDescParent()));
        gui.getParent().update((byte)31);
    }
